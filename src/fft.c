 ...
 * void pitchchange
 * 使い方
 *   pitchchange(int n, long npitch, short* buf)
 * n = sendするデータの数
 * npitch = 半音いくつ分変えたいか
 * buf = データ
 * 
...
///////pitch変更
void pitch(complex double * y, long n, int k){
    double f0 = 44100 / n;
    int k0 = 0;
    if(k >= 0){
        k0 = k;     
    ...
    double gap = (double)pow(2, (double)k0 / 12);
    if(k >= 0){
        for(int i = n - 1; i >= 0; i--){
            if(i * f0 <= f0 * gap){
                y[i] = 0;
...
void pitchchange(int n, long npitch, short* buf) {
  n = pow2(n);
  if (!pow2check(n)) {
